...
def eval_one_derived_model(
    ...
    """Evaluate one derived model against base model's bottom-k vocab."""
    device = args.device
    ...
    try:
        ...
        for idx, fp in enumerate(fingerprints):
            ...
            print(f"[pair {idx+1}/{len(fingerprints)}] overlap = {score:.4f}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bottom-k subspace overlap: fixed base model, test on derived models."
    )
    ...
    parser.add_argument("--csv_path", type=str, default="lineage_bottomk_overlap.csv")
...
def main() -> None:
    ...
    try:
        # 2) Generate or load fingerprints
        ...
        # 3) Precompute base model's bottom-k vocab
        print(f"[base] Computing bottom-k vocab for {len(fingerprints)} fingerprints...")
        base_bottomk_cache: Dict[str, List[int]] = {}
        for idx, fp in enumerate(fingerprints):
            prompt_text = extract_prompt_from_fingerprint(fp)
            base_bottomk_ids = compute_bottomk_vocab_for_model(
                base_model, base_tok, k=args.bottom_k_vocab, device=args.device, prompt=prompt_text
            )
            base_bottomk_cache[prompt_text] = base_bottomk_ids
            print(f"[base] Cached bottom-k for fingerprint {idx+1}/{len(fingerprints)}")

        # 4) Load derived models from CSV
