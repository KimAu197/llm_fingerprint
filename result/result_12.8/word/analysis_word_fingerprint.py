...
# Load data
diff_llama = pd.read_csv('lineage_bottomk_scores_llama_diff.csv')
diff_qwen = pd.read_csv('lineage_bottomk_scores_qwen_diff.csv')
...
bins = np.linspace(0, max(same_scores_qwen.max(), diff_scores_qwen.max()) + 0.05, 30)
...
ax.set_xlabel('avg_overlap_ratio', fontsize=12)
ax.set_ylabel('Count', fontsize=12)
ax.set_title('Qwen: Distribution of avg_overlap_ratio (Same vs Diff)', fontsize=14, fontweight='bold')
...
bins = np.linspace(0, max(same_scores_llama.max(), diff_scores_llama.max()) + 0.05, 30)
...
ax.set_xlabel('avg_overlap_ratio', fontsize=12)
ax.set_ylabel('Count', fontsize=12)
ax.set_title('LLaMA: Distribution of avg_overlap_ratio (Same vs Diff)', fontsize=14, fontweight='bold')
ax.legend(loc='upper right', fontsize=10)
ax.grid(True, alpha=0.3)
...
ax.set_title('ROC Curve: Qwen vs LLaMA (Word-based Fingerprint)', fontsize=14, fontweight='bold')
