#!/usr/bin/env python3
"""Plot strict fine-tuning lineage trees from model_lineage_data_clean.json (paginated PDF + PNG preview)."""
...
def write_singleton_csv(
    ...
    rows = []
    for c in clusters:
        if int(c.get("size", 0)) != 1:
            continue
        mid = c["models"][0]
        detail = models_by_name.get(mid, {})
        rows.append(
            {
...
def main() -> None:
    ...
    default_root = here.parent / "new_data"
    ap = argparse.ArgumentParser()
    ap.add_argument(
        "--lineage",
        type=Path,
        default=default_root / "model_lineage_data_clean.json",
    )
    ap.add_argument(
        "--out-pdf",
        type=Path,
        default=default_root / "strict_lineage_trees_clean.pdf",
    )
    ap.add_argument(
        "--out-png",
        type=Path,
        default=default_root / "strict_lineage_trees_clean.png",
    )
    ap.add_argument(
        "--singleton-csv",
        type=Path,
        default=default_root / "singleton_clusters_clean.csv",
    )
    ...
    models_by_name = {m["model_name"]: m for m in data.get("models", []) if m.get("model_name")}

    n_singleton = write_singleton_csv(clusters_all, models_by_name, args.singleton_csv)
    n_multi = sum(1 for c in clusters_all if int(c.get("size", 0)) > 1)
    ...
    suptitle = (
        "Strict fine-tuning lineage trees (HF card base_model, in-set edges only; "
        "overlap-matrix clean subset, n=183)"
    )
    ...
    print(f"Wrote {args.singleton_csv} ({n_singleton} single-model clusters)")
    print(f"Multi-model clusters: {n_multi}")
